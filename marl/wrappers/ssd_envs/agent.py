...
from marl.wrappers.ssd_envs.utility_funcs import return_view
...
class Agent:
  ...
  def __init__(self, agent_id, start_pos, start_orientation, full_map, row_size,
               col_size):
  ...
  def is_tile_walkable(self, row, column):
    return (0 <= row < self.full_map.shape[0] and
            0 <= column < self.full_map.shape[1]
            # You can't walk through walls, closed doors or switches
...
class HarvestAgent(Agent):
  ...
  def __init__(self, agent_id, start_pos, start_orientation, full_map,
               view_len):
...
CLEANUP_ACTIONS.update({
    7: "FIRE",
    8: "CLEAN"
})  # Fire a penalty beam  # Fire a cleaning beam
