...
def run_experiment(
  ...
  environment_specs: ma_specs.MAEnvironmentSpec = (
      experiment.environment_spec or ma_specs.MAEnvironmentSpec(environment))
  ...
  replay_tables = experiment.builder.make_replay_tables(environment_specs,
                                                        policy)
  ...
  train_counter = counting.Counter(
      parent_counter, prefix="train", time_delta=0.0)
  train_logger = experiment.logger_factory(
      label="train", steps_key=train_counter.get_steps_key(), task_instance=0)
  ...
  eval_counter = counting.Counter(parent_counter, prefix="eval", time_delta=0.0)
  eval_logger = experiment.logger_factory(
      label="eval", steps_key=eval_counter.get_steps_key(), task_instance=0)

  eval_policy = experiment.builder.make_policy(
      network, environment_specs, evaluation=True)
...
def _disable_insert_blocking(
  ...
  # Recent Reverb builds no longer expose table replacement in Python, so for
  # the local single-process runner we leave tables unchanged and simply wait
  # until at least one sample is available before triggering learner updates.
